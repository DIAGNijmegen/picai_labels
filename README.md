...
🚨 13 July 2026: We have released the [PI-RADS][PI-RADS] scores assigned during routine practice for each of the 1500 cases spanning the **Public Training and Development Dataset** of the [PI-CAI challenge](https://pi-cai.grand-challenge.org/). Researchers/developers can use these scores to estimate the difficulty of detecting prostate cancer on individual MRI examinations or to calibrate their AI models against radiologists’ assessments. Please note that this dataset was curated through convenience sampling. It is therefore not representative of the standard of care at the participating sites and should neither be interpreted as such nor used to benchmark AI systems.
...
| [csPCa_lesion_delineations/<br>human_expert/Pooch25/](csPCa_lesion_delineations/human_expert/Pooch25/) | 🚨 All-new human expert-derived annotations from [Pooch et al., 2025](https://www.medrxiv.org/content/early/2025/05/13/2025.05.13.25327456) for the 205 positive cases that were previously annotated by an AI system only ([Bosma et al., 2022a][Bosma22]). Combined with the pre-existing annotations for the remaining 1295 cases, researchers/developers can now use human expert-derived annotations for all 1500 cases.|
...
| ```lesion_PIRADS```🚨             | [PI-RADS][PI-RADS] v1/2/2.1 score (depending on the clinical guidelines in-effect at the time) assigned to each lesion during prostate MRI examination, where scores for different lesions are separated by ```,``` (commas). If its value is missing, then no suspicious lesion was identified; indicating a negative MRI study. If its value is ```N/A``` only for specific lesion(s), then those lesion(s) (as identified on histopathology) were not identified on MRI (typically the case for MRI-invisible lesions or lesions found using [systematic biopsies][SysBx] biopsies). 
| ```lesion_GS```             | [Gleason score (GS)][GS] assigned to each lesion after histopathologic analysis, where scores for different lesions are separated by ```,``` (commas). If its value is missing, then no tissue sampling procedure was performed; indicating a negative MRI study. If its value is ```N/A``` only for specific lesion(s), then those lesion(s) (as identified on MRI) were not biopsied or graded in histopathology (typically the case for [PI-RADS][PI-RADS] 1-2 lesions). |
